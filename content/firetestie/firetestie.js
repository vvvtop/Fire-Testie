          ...
          fireTestiePanel.prototype = FBL.extend(Firebug.Panel, function () {
                ...
                showPbox = function (ele1, ele2, x, y) {
                  ...
                  pbox.innerHTML = '<div class=" wrapper">' +
                    ...
                    '<div class="d5">' + parseInt(b.x + b.w - a.x - a.w) + 'px</div>' +
                    '<div class="d6">' + parseInt(b.y + b.h - a.y - a.h) + 'px</div>' +
                    '</div>';
                },
                  ...
                  NetUtil.asyncFetch(channel, function (inputStream, status) {
                      ...
                      for (var i in la) {
                        ...
                        matchTableTmp += ('<tr style="text-align:start;border-bottom:1px solid #333;">' +
                          ...
                          '<td style="width:12%;border-right:1px dashed #333;' +
                          'padding-left:7px;padding-top:3px;padding-bottom:3px;">' +
                          i + '</td><td style="padding-left:7px;padding-top:3px;padding-bottom:3px;"><input type="text" style="width:90%;" lastValue=\'' + JSON.stringify(la[i]) + '\' value_for="' + i + '" class="match_field" value=\'' +
                          JSON.stringify(la[i]) +
                          '\' /></td></tr>');
                ...
                show = function () {
                  ...
                  var matchTable = mTable = textWrapper.matchtable.append(null, rootTemplateElement, textWrapper);
                  ...
                  for (var styleName in styleNames) {
                    count += 1;
                    cssTableTmp += '<td><input id="' + styleNames[styleName][0]
                     + '_cssattr" type="checkbox" value="' + styleName
                     + '" name="cssattr" class="cssattr"'
                     + (styleNames[styleName][1] ? ' checked' : '')
                     + '><label for="'
